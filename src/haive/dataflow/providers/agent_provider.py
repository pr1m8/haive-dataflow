...
from haive.dataflow.providers.providers.base import EntityProvider
                                ...
                                try:
                                    ...
                                    if hasattr(instance, "engine"):
                                        ...
                                        if engine_type == "aug_llm":
                                            ...
                                            if hasattr(
                                                instance.engine, "prompt_template"
                                            ):
                                            ...
                                            # Extract tools
                                            if (
                                                hasattr(instance.engine, "tools")
                                                and instance.engine.tools
                                            ):
                                                # Register the tools
                                                tool_ids = []
                                                for i, tool in enumerate(
                                                    instance.engine.tools
                                                ):
                                                    if hasattr(tool, "name"):
                                                        tool_name = tool.name
                                                    else:
                                                        tool_name = (
                                                            f"{agent_name}_tool_{i}"
                                                        )
                                    ...
                                    # Additional engines
                                    if (
                                        hasattr(instance, "engines")
                                        and instance.engines
                                    ):
                                        for (
                                            engine_name,
                                            engine,
                                        ) in instance.engines.items():
                                            # Register the engine
                                            engine_id = registry_system.register_entity(
                                                name=engine_name,
                                                entity_type=EntityType.ENGINE,
                                    ...
                                    if (
                                        hasattr(instance, "agent_settings")
                                        and instance.agent_settings
                                    ):
                                        for (
                                            key,
                                            value,
                                        ) in instance.agent_settings.items():
