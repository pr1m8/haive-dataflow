...
class RegistrySystem:
    ...
    The registry system serves as a centralized repository for tracking and
    managing various components in the Haive ecosystem, such as agents, tools,
    engines, games, and LLM models. It provides both in-memory storage and
    optional database persistence via Supabase.

    Components can be registered, configured, queried, and managed through
    the registry system, which maintains metadata, configurations, and
    dependency relationships between components.

    Attributes:
        _entities (dict): In-memory storage for registry items
        _configurations (dict): In-memory storage for configurations
        _dependencies (dict): In-memory storage for dependencies
        _environment_vars (dict): In-memory storage for environment variables
    ...
    def __init__(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Could not ensure registry schema: {schema_e}")

            # Check for provider types in the new schema
            try:
                self._ensure_provider_types()
            except Exception as provider_e:
                logger.warning(f"Could not ensure provider types: {provider_e}")
    ...
    def _ensure_registry_schema(self):
        ...
        try:
            ...
            schema_check = self._supabase.rpc(
                "execute_sql",
                {
                    "sql": "SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'registry'"
                },
            ).execute()
            ...
            if not schema_check.data or len(schema_check.data) == 0:
                ...
                self._supabase.rpc(
                    "execute_sql", {"sql": "CREATE SCHEMA IF NOT EXISTS registry"}
                ).execute()
    ...
    def _ensure_provider_types(self):
        ...
        try:
            ...
            table_check = self._supabase.rpc(
                "execute_sql",
                {
                    "sql": "SELECT tablename FROM pg_catalog.pg_tables WHERE schemaname = 'models' AND tablename = 'provider_types'"
                },
            ).execute()
            ...
            if table_check.data and len(table_check.data) > 0:
                ...
                if (
                    not provider_types_response.data
                    or len(provider_types_response.data) == 0
                ):
                    ...
                    for provider_type in provider_types:
                        table(self._supabase, "models.provider_types").insert(
                            provider_type
                        ).execute()
            ...
            try:
                    ...
                    try:
                        ...
                        if provider_type_response:
                            ...
                            provider_data = {
                                ...
                                "is_available": (
                                    metadata.get("is_available", False)
                                    if metadata
                                    else False
                                ),
                ...
                if not success:
                    try:
                        ...
                        logger.warning(
                            f"Error storing {name} in config.components: {e}"
                        )
    ...
    def _get_or_create_provider_type(self, type_name, display_name):
        ...
        try:
            ...
            sql_response = self._supabase.rpc(
                "execute_sql",
                {
                    "sql": create_query.replace(
                        "$1", "'" + provider_type_data["id"] + "'"
                    )
                    .replace("$2", "'" + provider_type_data["name"] + "'")
                    .replace("$3", "'" + provider_type_data["display_name"] + "'")
                    .replace("$4", "'" + provider_type_data["description"] + "'")
                    .replace("$5", "'" + provider_type_data["created_at"] + "'")
                },
            ).execute()
        ...
        try:
            ...
            sql_response = self._supabase.rpc(
                "execute_sql",
                {
                    ...
                    .replace("$2", "'" + component_type_data["name"] + "'")
                    .replace("$3", "'" + component_type_data["display_name"] + "'")
                    .replace("$4", "'" + component_type_data["description"] + "'")
                    .replace("$5", "'" + component_type_data["created_at"] + "'")
                },
            ).execute()
    ...
    def add_configuration(
        ...
        if self._supabase is not None:
            try:
                ...
                table(self._supabase, "registry.configurations").insert(
                    db_config
                ).execute()
    ...
    def add_environment_var(
        ...
        """
        return self._register_environment_var(
            var_name, provider_name, is_required, description
        )
    ...
    def _register_environment_var(
        ...
        env_var_record = {
            ...
            "description": description
            or f"Environment variable {var_name} for {provider_name}",
            "created_at": datetime.now().isoformat(),
        }
            ...
            try:
                ...
                try:
                    ...
                    if not env_var_response.data or len(env_var_response.data) == 0:
                        ...
                        if response.data and len(response.data) > 0:
                            ...
                            try:
                                ...
                                if (
                                    ...
                                    and len(provider_response.data) > 0
                                ):
                        ...
                        table(self._supabase, "config.environment_variables").update(
                            env_var_data
                ...
                try:
                    ...
                    """
                    self._supabase.rpc(
                        "execute_sql", {"sql": create_env_vars_table}
                    ).execute()

                    # Check if record already exists
                    response = (
                        table(self._supabase, "registry.environment_vars")
                        .select("*")
                    ...
                    logger.warning(
                        f"Error adding to registry.environment_vars: {legacy_e}"
                    )
        ...
        if self._supabase is not None:
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Error storing import log in audit.import_logs: {e}"
                    )

                    # Check if it's a table not found error
                    if (hasattr(e, "code") and e.code == "42P01") or str(e).find(
                        "does not exist"
                    ) >= 0:
                        ...
                        try:
                            ...
                            """
                            self._supabase.rpc(
                                "execute_sql", {"sql": create_table_query}
                            ).execute()
                ...
                try:
                    ...
                    self._supabase.rpc(
                        "execute_sql",
                        {
                            "sql": insert_query.replace(
                                "$1", "'" + log_entry["id"] + "'"
                            )
                            .replace("$2", "'" + log_entry["import_session"] + "'")
                            .replace("$3", "'" + log_entry["entity_name"] + "'")
                            .replace("$4", "'" + log_entry["entity_type"] + "'")
                            .replace("$5", "'" + log_entry["status"] + "'")
                            .replace("$6", "'" + (log_entry["message"] or "") + "'")
                            .replace("$7", "'" + (log_entry["traceback"] or "") + "'")
                            .replace("$8", "'" + log_entry["created_at"] + "'")
                        },
                    ).execute()
                    logger.info(
                    ...
                    try:
                        ...
                        """
                        self._supabase.rpc(
                            "execute_sql", {"sql": create_legacy_table_query}
                        ).execute()

                        # Insert into legacy table
                        table(self._supabase, "registry.import_logs").insert(
                            log_entry
                        ).execute()
                        logger.info(
    ...
    def get_entities_by_type(self, entity_type: EntityType) -> list[dict[str, Any]]:
                ...
                if response.data:
                    ...
                    if entity_type_value in [
                        ...
                        for entity in response.data:
                            ...
                            required_vars = [
                                env for env in env_vars if env["is_required"]
                            ]

                            if required_vars:
                                # Set availability based on required env vars
                                all_required_available = all(
                                    self.check_environment_var(env["var_name"])
        ...
        return [
            ...
            if entity["type"] == entity_type_value
        ]
    ...
    def get_environment_vars(
        ...
        if self._supabase is not None:
            try:
                ...
                try:
                    query = table(
                        self._supabase, "config.environment_variables"
                    ).select("*")
        ...
        if provider_name:
            return [
                var
                for var in self._environment_vars.values()
                if var["provider_name"] == provider_name
            ]
    ...
    def get_available_providers(
        ...
        if entity_type_value:
            ...
            # Get both types
            providers = self.get_entities_by_type(
                EntityType.LLM_PROVIDER
        ...
        for provider in providers:
            ...
            if required_vars:
                # Set availability based on required env vars
                all_required_available = all(
                    self.check_environment_var(env["var_name"]) for env in required_vars
                )
                provider["is_available"] = all_required_available
            else:
                # No required env vars, assume available
                    ...
                    if metadata_filter:
                        ...
                        for entity in response.data:
                            ...
                            for key, value in metadata_filter.items():
                                if (
                                    key not in entity_metadata
                                    or entity_metadata[key] != value
                                ):
