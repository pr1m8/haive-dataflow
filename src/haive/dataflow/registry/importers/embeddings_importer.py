        ...
        if registry_system._supabase is not None:
            ...
            for model_info in EMBEDDING_MODELS:
                try:
                    ...
                    response = (
                        table(registry_system._supabase, "agents.embedding_models")
                        .select("*")
                    ...
                    if response.data and len(response.data) > 0:
                        ...
                        table(
                            registry_system._supabase, "agents.embedding_models"
                        ).update(model_data).eq("id", model_id_pk).execute()
                    else:
                        # Insert new model
                        model_response = (
                            table(registry_system._supabase, "agents.embedding_models")
                    ...
                    pricing_response = (
                        table(registry_system._supabase, "agents.embedding_pricing")
                        .select("*")
                        .eq("model_id", model_id)
                        .execute()
                    )
                    ...
                    if pricing_response.data and len(pricing_response.data) > 0:
                        ...
                        table(
                            registry_system._supabase, "agents.embedding_pricing"
                        ).update(pricing_data).eq("id", pricing_id).execute()
                    else:
                        # Insert new pricing
                        table(
                            registry_system._supabase, "agents.embedding_pricing"
                        ).insert(pricing_data).execute()
