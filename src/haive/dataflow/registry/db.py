...
class RegistryDB:
    ...
    def _create_registry_table(self, table_name: str):
        ...
        try:
            ...
            try:
                self._supabase_client.table(table_name).select(
                    "count(*)", count="exact"
                ).limit(1).execute()
    ...
    def upsert_registry_item(self, item: RegistrySchema) -> str | None:
        ...
        try:
            ...
            if "timestamp" in item_dict and isinstance(
                item_dict["timestamp"], datetime
            ):
    ...
    def get_registry_item(self, name: str, item_type: str) -> RegistrySchema | None:
        ...
        try:
            ...
            if response.data:
                for item_data in response.data:
                    # Parse metadata if needed
                    if "metadata" in item_data and isinstance(
                        item_data["metadata"], str
                    ):
                        item_data["metadata"] = json.loads(item_data["metadata"])
    ...
    def upsert_schema_definition(self, schema: SchemaDefinition) -> str | None:
        ...
        try:
            ...
            if "schema_json" in schema_dict and isinstance(
                schema_dict["schema_json"], dict
            ):
                schema_dict["schema_json"] = json.dumps(schema_dict["schema_json"])

            # Convert timestamp to string if needed
            if "timestamp" in schema_dict and isinstance(
                schema_dict["timestamp"], datetime
            ):
                schema_dict["timestamp"] = schema_dict["timestamp"].isoformat()

            # Upsert the schema
            response = (
                self._supabase_client.table("component_schemas")
                .upsert(schema_dict)
                .execute()
            )
    ...
    def get_schema_definitions(
        self, registry_item_id: str, schema_type: str | None = None
        ...
        try:
            ...
            if response.data:
                for schema_data in response.data:
                    ...
                    if "schema_json" in schema_data and isinstance(
                        schema_data["schema_json"], str
                    ):
                        schema_data["schema_json"] = json.loads(
    ...
    def upsert_agent_graph(self, graph: AgentGraph) -> str | None:
        ...
        try:
            ...
            # Convert timestamp to string if needed
            if "timestamp" in graph_dict and isinstance(
                graph_dict["timestamp"], datetime
            ):
                graph_dict["timestamp"] = graph_dict["timestamp"].isoformat()

            # Upsert the graph
            response = (
                self._supabase_client.table("agent_graphs").upsert(graph_dict).execute()
            )
    ...
    def get_agent_graph(self, agent_id: str) -> AgentGraph | None:
        ...
        try:
            # Query the graph
            response = (
                self._supabase_client.table("agent_graphs")
                .select("*")
...
# Export the instance
__all__ = ["AgentGraph", "RegistrySchema", "SchemaDefinition", "registry_db"]
