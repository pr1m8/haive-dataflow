...
from haive.dataflow.supabase.client import get_supabase_client
...
class SupabaseAuthManager:
    ...
    def __init__(self, config: SupabaseAuthConfig | None = None):
    ...
    def _verify_jwt_direct(self, token: str) -> dict[str, Any]:
        ...
        try:
            ...
            if "exp" in decoded_token and datetime.now(UTC).timestamp() > decoded_token["exp"]:
            ...
            logger.warning(f"Direct JWT verification failed: {e!s}, falling back to API verification")
            # Fall back to API verification
            return self._verify_jwt_api(token)
    ...
    def refresh_token(self, refresh_token: str) -> dict[str, str]:
        ...
        try:
            ...
            raise ValueError(f"Token refresh failed: {e!s}")

# Singleton instance
_auth_manager = None

def get_auth_manager(config: SupabaseAuthConfig | None = None) -> SupabaseAuthManager:
