...
from haive.dataflow.auth.config.environment import get_supabase_server_config
...
class CreditsManager:
    ...
    async def check_credits(self, user_id: str, required_credits: float = 1.0) -> bool:
        ...
        try:
            ...
            response = (
                await self.client.from_("user_data.credits")
    ...
    async def deduct_credits(self, user_id: str, amount: float) -> bool:
        ...
        try:
            ...
            response = await self.client.rpc(
                "deduct_user_credits", {"p_user_id": user_id, "p_amount": amount}
            ).execute()
    ...
    async def log_usage(self, usage: UsageRecord) -> bool:
        ...
        try:
            # Insert usage record
            response = (
                await self.client.from_("user_data.usage")
                .insert(
                    {
