...
def create_agent_router(prefix: str = "/agents") -> APIRouter:
    ...
    async def create_thread(
        ...
        initial_state: dict[str, Any] | None = Body(
    ...
    async def get_thread_state(
        ...
        agent = agent_registry.get_or_create_agent(
            agent_name, thread_id=thread_id, **config
        )
        if not agent:
            raise HTTPException(status_code=404, detail=f"Agent {agent_name} not found")
    ...
    async def send_text(
        ...
        if hasattr(agent, "state_schema") and hasattr(
            agent.state_schema, "__annotations__"
        ):
        ...
        if has_messages:
            ...
            for field in ["input", "text", "query", "content"]:
                if (
                    hasattr(agent, "state_schema")
                    and hasattr(agent.state_schema, "__annotations__")
                    and field in agent.state_schema.__annotations__
                ):
        ...
        if hasattr(agent_registry, "db") and agent_registry.db_connection:
            try:
                with agent_registry.db.connection.cursor() as cursor:
                    cursor.execute("SELECT * FROM ai.agent_types ORDER BY name")
                    types = [
