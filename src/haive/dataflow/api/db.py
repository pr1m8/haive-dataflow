...
class DatabaseManager:
    ...
    def create_tables(self) -> bool:
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                self.connection.commit()
                logger.info(
                    f"Tables created successfully in schema '{self.schema_name}'"
                )
                return True
        except Exception as e:
    ...
    def register_agent_config(
        self, name: str, class_obj: Any, agent_type: str = "agent"
    ) -> bool:
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                if not agent_type_id:
                    cursor.execute(
                        sql.SQL(
                            "INSERT INTO {}.agent_types (name) VALUES (%s) RETURNING id"
                        ).format(sql.Identifier(self.schema_name)),
                        (agent_type,),
                    )
                    agent_type_id = cursor.fetchone()
    ...
    def get_agent_configs(self, agent_type: str | None = None) -> list[dict[str, Any]]:
