...
from haive.dataflow.api.llms.api.llms.models import Provider
...
app = FastAPI(
    title="LLM Model API", description="API for LLM model information and availability"
)
...
def get_models(provider: str | None = None) -> list[dict[str, Any]]:
    ...
    for model in models:
        ...
        # Get capabilities
        cap_response = (
            supabase.table("llm_model_capabilities")
            .select("*")
        ...
        # Get pricing
        price_response = (
            supabase.table("llm_pricing").select("*").eq("model_id", model_id).execute()
        )
        pricing = price_response.data[0] if price_response.data else {}

        # Get search pricing if applicable
        search_response = (
            supabase.table("llm_search_pricing")
            .select("*")
            .eq("model_id", model_id)
            .execute()
        )
...
def get_model_by_id(model_id: str) -> dict[str, Any] | None:
    ...
    # Get model
    model_response = (
        supabase.table("llm_models").select("*").eq("model_id", model_id).execute()
    )
...
async def read_models(provider: str | None = None, capability: str | None = None):
...
async def recommended_models(
    ...
    function_calling: bool | None = False,
    ...
    for model in all_models:
        ...
        if function_calling and not capabilities.get(
            "supports_function_calling", False
        ):
            continue

        if audio and not (
            capabilities.get("supports_audio_input", False)
            or capabilities.get("supports_audio_output", False)
        ):
            continue
        ...
        provider_response = (
            ...
            .select("is_available")
            .eq("name", model["provider"])
            .execute()
        )
        if provider_response.data and not provider_response.data[0].get(
            "is_available", False
        ):
