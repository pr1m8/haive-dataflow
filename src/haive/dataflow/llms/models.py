...
class Pricing(BaseModel):
    ...
    input_cost_per_token_batches: float | None = Field(0, description="Discounted input cost for batched requests")
    output_cost_per_token_batches: float | None = Field(0, description="Discounted output cost for batched requests")
    input_cost_per_audio_token: float | None = Field(0, description="Cost per audio input token (e.g., for speech-to-text)")
    output_cost_per_audio_token: float | None = Field(0, description="Cost per audio output token (e.g., for TTS)")
    cache_read_input_token_cost: float | None = Field(0, description="Cost for reading cached prompt tokens, if applicable")
...
class Model(BaseModel):
    ...
    deprecation_date: str | None = Field(None, description="Optional deprecation or sunset date for the model")
    capabilities: ModelCapabilities | None = Field(None, description="Feature support map for the model")
    pricing: Pricing | None = Field(None, description="Pricing information for token or usage cost")
    search_pricing: SearchPricing | None = Field(None, description="Search task-related pricing details")
