...
from haive.core.persistence.factory import acreate_postgres_checkpointer, aregister_postgres_thread, aget_postgres_checkpoint, aput_postgres_checkpoint
...
class ConversationManager:
    ...
    def __init__(
        self, 
        server_config: Optional[SupabaseServerConfig] = None,
        postgres_config: Optional[PostgresCheckpointerConfig] = None
    ):
    ...
    async def create_conversation(
        ...
        try:
            ...
            # Register thread
            success = await aregister_postgres_thread(checkpointer, thread_id, thread_metadata)
    ...
    async def get_conversation_state(
        ...
        try:
            ...
            if not response.data:
                logger.warning(f"User {user_id} attempted to access unauthorized thread: {thread_id}")
                return None
    ...
    async def update_conversation_state(
        ...
        try:
            ...
            if not response.data:
                logger.warning(f"User {user_id} attempted to update unauthorized thread: {thread_id}")
                return False
