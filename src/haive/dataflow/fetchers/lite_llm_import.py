            ...
            try:
                ...
                if env_var:
                    registry_system.add_environment_var(
                        registry_id=provider_id, env_name=env_var, is_required=True
                    )
                ...
                logger.info(
                    f"Registered provider: {provider} (available: {is_available})"
                )
        ...
        if registry_system._supabase is not None:
            ...
            for model_id, model_info in model_data.items():
                ...
                try:
                    ...
                    if not provider:
                        logger.warning(
                            f"Could not determine provider for model {model_id}"
                        )
                        continue
                    ...
                    capabilities = {
                        ...
                        "supports_audio_input": model_info.get(
                            "supports_audio_input", False
                        ),
                        "supports_audio_output": model_info.get(
                            "supports_audio_output", False
                        ),
                        "supports_prompt_caching": model_info.get(
                        ...
                        "supports_web_search": model_info.get(
                            "supports_web_search", False
                        ),
                        "supports_tool_choice": model_info.get(
                    ...
                    pricing = {
                        ...
                        "output_cost_per_token": model_info.get(
                        ...
                        "input_cost_per_audio_token": model_info.get(
                            "input_cost_per_audio_token", 0
                        ),
                        "output_cost_per_audio_token": model_info.get(
                    ...
                    response = (
                        registry_system._supabase.table("agents.llm_models")
                        .select("*")
                    ...
                    if response.data and len(response.data) > 0:
                        ...
                        registry_system._supabase.table("agents.llm_models").update(
                            model_data
                        ).eq("id", model_id_pk).execute()
                    else:
                        # Insert new model
                        model_response = (
                            registry_system._supabase.table("agents.llm_models")
                    ...
                    capabilities_response = (
                        registry_system._supabase.table("agents.llm_model_capabilities")
                        .select("*")
                        .eq("model_id", model_id)
                        .execute()
                    )
                    ...
                    if (
                        ...
                        registry_system._supabase.table(
                            "agents.llm_model_capabilities"
                        ).update(capabilities_data).eq("id", capabilities_id).execute()
                    else:
                        # Insert new capabilities
                        registry_system._supabase.table(
                            "agents.llm_model_capabilities"
                        ).insert(capabilities_data).execute()
                    ...
                    pricing_response = (
                        registry_system._supabase.table("agents.llm_pricing")
                        .select("*")
                        .eq("model_id", model_id)
                        .execute()
                    )
                    ...
                    if pricing_response.data and len(pricing_response.data) > 0:
                        ...
                        registry_system._supabase.table("agents.llm_pricing").update(
                            pricing_data
                        ).eq("id", pricing_id).execute()
                    else:
                        # Insert new pricing
                        registry_system._supabase.table("agents.llm_pricing").insert(
                            pricing_data
                        ).execute()
                    ...
                    if "search_context_cost_per_query" in model_info:
                        search_pricing = {
                            "model_id": model_id,
                            "search_context_size_low": model_info.get(
                                "search_context_cost_per_query", {}
                            ).get("search_context_size_low", 0),
                            "search_context_size_medium": model_info.get(
                                "search_context_cost_per_query", {}
                            ).get("search_context_size_medium", 0),
                            "search_context_size_high": model_info.get(
                                "search_context_cost_per_query", {}
                            ).get("search_context_size_high", 0),
                            "created_at": datetime.now().isoformat(),
                            "updated_at": datetime.now().isoformat(),
                        }

                        search_pricing_response = (
                            registry_system._supabase.table("agents.llm_search_pricing")
                            .select("*")
                            .eq("model_id", model_id)
                            .execute()
                        )
                        ...
                        if (
                            search_pricing_response.data
                            and len(search_pricing_response.data) > 0
                        ):
                            # Update existing search pricing
                            search_pricing_id = search_pricing_response.data[0]["id"]
                            registry_system._supabase.table(
                                "agents.llm_search_pricing"
                            ).update(search_pricing).eq(
                                "id", search_pricing_id
                            ).execute()
                        else:
                            # Insert new search pricing
                            registry_system._supabase.table(
                                "agents.llm_search_pricing"
                            ).insert(search_pricing).execute()
            ...
            for model_id, model_info in model_data.items():
                ...
                try:
                    ...
                    if model_info.get("max_tokens"):
                        description += (
                            f", context window: {model_info.get('max_tokens')} tokens"
                        )
