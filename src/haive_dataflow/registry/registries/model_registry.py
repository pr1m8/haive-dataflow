...
from haive_dataflow.db.supabase import get_supabase_client, table

# Import LLM core models for environment variable inspection
try:
    from haive_core.models.llm.base import SecureConfigMixin, LLMConfig
...
class ModelRegistry:
    ...
    def _load_model_data(self):
        ...
        try:
            from haive_dataflow.registry.importers.embeddings_importer import EMBEDDING_MODELS
    ...
    def get_available_llm_providers(self) -> List[Dict[str, Any]]:
        ...
        if self._supabase:
            try:
                ...
                try:
                    from haive_dataflow.db.supabase import table
    ...
    def get_llm_models(self, provider: Optional[str] = None, only_available: bool = False) -> List[Dict[str, Any]]:
        ...
        if not results:
            try:
                ...
                from haive_dataflow.registry.importers.litellm_importer import import_llm_models
    ...
    def get_embedding_models(self, provider: Optional[str] = None, only_available: bool = False) -> List[Dict[str, Any]]:
        ...
        if not results and not self._embedding_models_cache:
            try:
                ...
                from haive_dataflow.registry.importers.embeddings_importer import import_embedding_models, EMBEDDING_MODELS
