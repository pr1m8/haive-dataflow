...
async def test_config():
    ...
    get_supabase_client_config = await safe_import("haive.dataflow.config.environment", "get_supabase_client_config")
    get_supabase_server_config = await safe_import("haive.dataflow.config.environment", "get_supabase_server_config")
    ...
    try:
        ...
        logger.info(f"Anon key present: {'Yes' if client_config.anon_key.get_secret_value() else 'No'}")
        
        # Test server config
        server_config = get_supabase_server_config()
        logger.info(f"Supabase URL: {server_config.url}")
        logger.info(f"Service key present: {'Yes' if server_config.service_role_key.get_secret_value() else 'No'}")
        logger.info(f"JWT secret present: {'Yes' if server_config.jwt_secret.get_secret_value() else 'No'}")
...
async def test_conversation(user_id):
    ...
    # Import classes
    ConversationManager = await safe_import("haive_dataflow.persistence.conversations", "ConversationManager")
    ConversationMetadata = await safe_import("haive_dataflow.persistence.conversations", "ConversationMetadata")
...
async def test_persistence(user_id, thread_id):
    ...
    # Import class
    SupabasePersistence = await safe_import("haive_dataflow.persistence.supabase_adapter", "SupabasePersistence")
...
async def main():
    ...
    connection_result = await run_with_error_handling("test_supabase_connection", test_supabase_connection)
    ...
    # Test credits
    credits_result = await run_with_error_handling("test_credits", test_credits, user_id)
    
    # Test conversation
    thread_id = await run_with_error_handling("test_conversation", test_conversation, user_id)
    if not thread_id:
        logger.error("Conversation test failed. Stopping further tests.")
        return
    
    # Test persistence
    persistence_result = await run_with_error_handling("test_persistence", test_persistence, user_id, thread_id)
